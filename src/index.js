...
const imageTargetPipelineModule = () => {
  const modelFile = "example-model.glb";
  ...
  const showTarget = ({ detail }) => {
    // When the image target named 'card-target' is detected, show 3D model.
    // This string must match the name of the image target uploaded to 8th Wall.
    if (detail.name === "card-target") {
    ...
    // When the image target named 'card-target' is detected, play video.
    // This string must match the name of the image target uploaded to 8th Wall.
    // if (detail.name === "card-target") {
