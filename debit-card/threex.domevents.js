...
THREEx.DomEvents.prototype.addEventListener = function(
  object,
  eventType,
  callback,
  useCapture
...
THREEx.DomEvents.prototype.removeEventListener = function(
  object,
  eventType,
  callback,
  useCapture
 ...
 */
THREEx.DomEvents.prototype._processMouseMove = function(
  pointerContext,
  intersects,
  eventType
) {
  ...
  if (
    intersectObject !== pointerContext.lastMouseMoveObject &&
    pointerContext.lastMouseMoveObject !== null
  ) {
    for (
      var object = pointerContext.lastMouseMoveObject;
      object !== null;
  ...
  function notifyAllListeners(object, event) {
    ...
    object.userData.listeners &&
      object.userData.listeners[event.type]
        .slice(0)
 ...
 */
THREEx.DomEvents.prototype.processIntersects = function(
  pointerContext,
  intersects,
  eventType
) {
  // sanity check
  console.assert(
    ["mousedown", "mouseup", "mousemove", "click"].indexOf(eventType) !== -1
  );
